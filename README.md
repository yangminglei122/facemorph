...
4. 上传照片或指定照片目录，点击"开始生成"

### 命令行快速使用

```bash
python main.py --input_dir ./images --output_dir ./output --sort name
```
...
- 支持多种参考图选项：
  - 自动选择：系统自动选择质量最高的图像作为参考图
  - 指定索引号：手动输入图像索引号作为参考图
  - 使用系统内部预定参考图：使用项目目录下的 `./ref/ref_image.jpg` 作为参考图
...
当图像数量超过 `--batch_size` 参数设定值（默认50张）时，程序会自动启用分批处理模式：

- **正常模式**：图像数量 ≤ batch_size，所有图像同时加载到内存
- **分批模式**：图像数量 > batch_size，分批加载和处理，避免内存不足

#### 分批处理流程

1. **参考图选择**：先分析所有批次，选择最佳参考图
2. **分批处理**：每批处理指定数量的图像
3. **内存管理**：处理完每批后立即释放内存
4. **结果合并**：所有批次处理完成后合并结果

#### 内存使用优化

- 及时释放已处理的图像数据
- 使用垃圾回收（gc.collect()）主动回收内存
- 监控内存使用情况（需要安装psutil）
- 可调节批次大小平衡内存和性能
...
```bash
# 内存充足时（推荐）
python main.py --input_dir ./images --output_dir ./output --batch_size 100

# 内存有限时
python main.py --input_dir ./images --output_dir ./output --batch_size 20

# 处理超大量图像时
python main.py --input_dir ./images --output_dir ./output --batch_size 10
```

### 性能优化建议

- **GPU加速**：使用 `--use_gpu` 启用CUDA光流加速
- **图像尺寸**：适当降低输出分辨率可显著减少内存占用
- **批次大小**：根据系统内存调整batch_size参数
- **并行处理**：Web界面支持多进程处理
...
1. **眼部水平度**（权重40%）：检查双眼是否水平
2. **鼻子对称性**（权重20%）：检查鼻子是否对称
3. **嘴巴对称性**（权重15%）：检查嘴部是否对称
4. **下颌对称性**（权重10%）：检查下颌是否对称
5. **整体对称性**（权重15%）：综合评估面部对称性

评分范围0-1，越高越好。手动指定参考图时会显示质量评分。

### 图像对齐算法

1. **参考图选择**：基于质量评分系统选择最佳参考图
...
7. **内存不足导致死机**：
   ...
   - **原因**：图像数量过多，内存不足
   - **解决**：
     - 使用 `--batch_size` 参数控制批次大小
     - 降低输出分辨率（`--width` 和 `--height`）
     - 减少同时处理的图像数量
   - **建议**：
     - 8GB内存：建议batch_size ≤ 30
     - 16GB内存：建议batch_size ≤ 50
     - 32GB内存：建议batch_size ≤ 100
...
- 改进了质量评分算法，更准确地评估图像正面程度
- 增加了对眼部、鼻子、嘴巴、下颌等多个面部特征的综合评估
- 添加了参考图质量评分显示，帮助用户选择最佳参考图

### 图像对齐算法改进

- 优化了眼部中心对齐算法，提高对齐精度
- 增加了后处理优化步骤，进一步减少人脸倾斜问题
- 改进了旋转角度计算，使用多个面部特征点进行综合评估
...
- 新增多种参考图选项：
  - 自动选择（默认）
  - 手动指定索引号
  - 使用系统内部预定参考图（`./ref/ref_image.jpg`）
  - 上传指定参考图
- 改进了进度显示，增加预计剩余时间
- 优化了结果预览功能，支持GIF和MP4播放

### 性能优化

- 优化了内存管理，减少内存占用
- 改进了分批处理算法，提高处理效率
- 增加了GPU加速支持，显著提升光流计算速度
