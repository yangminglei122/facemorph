...
2. **分批处理**：每批处理指定数量的图像
3. **内存管理**：处理完每批后立即释放内存
4. **错误日志**：在批处理过程中，如果某张图像的人脸检测或关键点检测失败，程序会输出详细的日志信息，包括图像的全局索引和文件名，便于用户定位问题图像
...
- **流式保存**：处理大量图像时使用 `--streaming_save` 参数
- **并行处理**：Web界面支持多进程处理
...
6. **未检测到人脸关键点**：程序会显示具体文件名，检查该图像是否有人脸或人脸是否清晰。在批处理模式下，程序会输出详细的日志信息，包括图像索引和文件名，便于定位问题图像。

7. **内存不足导致死机**：
   - **现象**：处理大量图像时程序崩溃或系统死机
   - **原因**：图像数量过多，内存不足
   - **解决**：
     - 使用 `--batch_size` 参数控制批次大小（默认50）
     - 降低输出分辨率（`--width` 和 `--height`）
     - 减少同时处理的图像数量
     - 使用 `--streaming_save` 参数启用流式保存功能
   - **建议**：
     - 8GB内存：建议batch_size ≤ 30
     - 16GB内存：建议batch_size ≤ 50
     - 32GB内存：建议batch_size ≤ 100
   - **技术细节**：程序会根据图像数量自动选择处理模式：
     - 图像数量 ≤ batch_size：使用正常处理模式，所有图像同时加载到内存
     - 图像数量 > batch_size：自动启用分批处理模式，分批加载和处理图像以避免内存不足
...
- 添加了对无效图像的检测和处理
- 修复了不生成GIF时的路径错误问题
- 改进了批处理模式下的人脸检测失败日志输出，现在会显示图像的全局索引和文件名，便于用户定位问题图像
